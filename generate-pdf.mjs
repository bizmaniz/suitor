...
function profileRootFromEnv() {
  const value = process.env.SUITOR_PROFILE_ROOT || process.env.SUITOR_PROFILE_ROOT || '';
...
async function generatePDF() {
  ...
  const profileRoot = profileRootFromEnv();
  if (profileRoot && !pathIsSameOrUnder(outputPath, profileRoot)) {
    console.error(`Refusing to write PDF outside SUITOR_PROFILE_ROOT: ${outputPath}`);
    process.exit(1);
  }
  ...
  // Resolve font paths relative to Suitor fonts.
